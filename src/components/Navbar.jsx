...
const linkClass = ({ isActive }) =>
    `text-sm font-medium transition-colors hover:text-white ${isActive ? "text-emarald-500" : "text-emerald-50"
    }`;
        ...
        <>
            <nav className='bg-emerald-100 text-emarald-800 border-b border-emerald-100 flex justify-between items-center p-3 gap-3 border-2 rounded shadow-lg shadow-gray-100'>
                ...
                <ul className='flex space-x-4 hidden md:flex'>
                    <li>  <Link to="/" className={`hover:text-emerald-600 transition-colors ${location.pathname === "/" ? "underline underline-offset-4 decoration-2" : ""
                            }`}
                    >
                        Home
                    </Link></li>
                    <li><Link to="/Products" className={`hover:text-emerald-600 cursor-pointer transition-colors ${location.pathname==='/Products' ?'underline underline-offset-4': ""}`}>Products</Link></li>
                    <li><NavLink to="/Cart" className={({isActive})=>`hover:text-emerald-600 cursor-pointer transition-color ${isActive ? "underline underline-offset-4 decoration-2" : ""}`}>Cart</NavLink></li>
                </ul>
